...
/**
 * This is the official Porter Stemming algorithm, retrieved from http://tartarus.org/~martin/PorterStemmer/ 
 */
...
public class Stemmer
{
	...
	/**
	 * Same as {@link #add(char[], int)}, but lets the user choose an offset (useful for large char arrays). Note that 
	 * this method will forcibly convert the chars in w to lower case. ~ Clemens A.
	 * @param w
	 * @param offset
	 * @param wLen
	 */
	public void add(char[] w, int offset, int wLen)
	{
		if (i + wLen >= b.length)
		{
			char[] new_b = new char[i + wLen + INC];
		...
		for (int c = offset; c < offset + wLen; c++)
			b[i++] = Character.toLowerCase(w[c]);
	}
	 ...
	 * file-name ...
	 */
	// Grp L: Commented since we don't need it and don't want to mix up this main method with ours.
	/*
